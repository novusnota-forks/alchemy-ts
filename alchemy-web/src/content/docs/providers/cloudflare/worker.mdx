...
By default, destroying an Alchemy scope deletes the Worker from Cloudflare. Set `delete: false` to remove the resource from Alchemy state without deleting the actual Worker:
...
This is useful when you want to hand off management of a Worker to another tool or process, or to prevent accidental deletion of critical infrastructure.
